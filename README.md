...
**「待验证」前置规则**：状态改为「待验证」要求**至少有一个端口已完成**——

- 详情页把状态改为「待验证」时若无任何端口完成，会被拦截并提示
- 新建条目不可能有已完成端口，因此初始状态不提供「待验证」
- 已处于「待验证」的条目，取消最后一个完成标记（或移除唯一完成端口对应的适用端口）时，状态**自动退回「开发中」**并给出提示
- 规则在前后端各有一份同语义实现（`src/utils/ports.ts` 与 `server/src/verifying-rule.js` 的 `applyVerifyingRule`），后端接口同样强制校验

## 列表排序

主列表按「状态优先 + 时间倒序」排列，让需要行动的条目始终在前：

1. 状态顺序：待处理 → 重新处理 → 开发中 → 待验证 → 验证通过（权重定义在 `src/constants.ts` 的 `STATUS_SORT_ORDER`）
2. 同一状态内：创建时间新的在前
3. 归档视图不按状态排（状态恒为已归档），只按创建时间倒序
